...
mongoose.connect(
  "mongodb://pivithuru:sysco19@ds161804.mlab.com:61804/project_db",
  { useMongoClient: true }
);
    ...
    if (err) {
      ...
      if (user) {
        ...
        if (passwordHash.verify(password, user.password)) {
          var token = jwt.sign({ email: email }, "secret");
          res.send({ status: "login Okay!", token: token });
        } else {
          res.send({ status: "Invalid Credentials!", token: "" });
        }
      } else {
        res.send({ status: "No Such a User!", token: "" });
      }
    }
  });
});
